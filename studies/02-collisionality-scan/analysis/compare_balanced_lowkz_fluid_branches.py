"""Compare fluid-only balanced low-kz forcing branches on the production grid."""
...
def cascade_fraction(k_perp: np.ndarray, e_k: np.ndarray, k_max_forced: int, Lx: float) -> float:
    if len(k_perp) != len(e_k):
        return float("nan")
    n_perp = k_perp * Lx / (2.0 * np.pi)
    forced_cut = float(k_max_forced) + 1e-9
    mask = n_perp > forced_cut
    total = float(np.sum(e_k[(k_perp > 0) & (e_k > 0)]))
    above = float(np.sum(e_k[mask & (e_k > 0)]))
    if total == 0.0:
        return float("nan")
    return above / total
...
def run_case(case: dict) -> dict:
    ...
    return {
        ...
        "cascade_frac": cascade_fraction(np.array(k_perp), np.array(e_k), case["k_max"], grid.Lx),
...
def plot() -> Path:
    ...
    for result in results:
        print(
            ...
            f"cascade_frac={result['cascade_frac']:.3e}"
        )
    ...
    fig.suptitle(
        "Balanced low-kz fluid-only branches at 64^2 x 32, M=0",
        fontsize=13,
    )
    fig.tight_layout()

    FIGURES_DIR.mkdir(parents=True, exist_ok=True)
    png_path = FIGURES_DIR / "balanced_lowkz_fluid_branches.png"
    pdf_path = FIGURES_DIR / "balanced_lowkz_fluid_branches.pdf"
