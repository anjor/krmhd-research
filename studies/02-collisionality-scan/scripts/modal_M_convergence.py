"""M-convergence study at nu=3 for the dissipative-anomaly paper.

Resume from the eta=100 steady Alfvenic checkpoint (the same t=2000 tau_A
checkpoint used by all hermite128_nu*_imex runs), expand M to one of the
target resolutions, add g_0 forcing, and measure eps_nu(M) at nu=3:

    - hermite_M64_nu3_imex:  M=64,  200 tau_A from t=2000
    - hermite_M256_nu3_imex: M=256, 100 tau_A from t=2000 (shorter to fit in
                              one Modal call; averaging window is still
                              substantially longer than the ~30 tau_A
                              Hermite fill-in transient)

Together with the existing hermite128_nu3_imex (M=128) run, this gives a
three-point M scan at the headline nu=3 point.

Usage:
    uv run modal run studies/02-collisionality-scan/scripts/modal_M_convergence.py
"""
from __future__ import annotations

import modal

app = modal.App("krmhd-hermite-M-convergence")
...
# M-convergence branches. nu=3, Alfvenic checkpoint shared.
BRANCHES = [
    {"label": "hermite_M64_nu3_imex",  "M": 64,  "nu": 3.0, "hermite_amplitude": 0.0035,
     "total_time": 2200, "averaging_start": 2100,
     "resume_from": "alfven128_lowkz_f0p02_eta100/checkpoints/checkpoint_t2000.0.h5"},
    {"label": "hermite_M256_nu3_imex", "M": 256, "nu": 3.0, "hermite_amplitude": 0.0035,
     "total_time": 2100, "averaging_start": 2050,
     "resume_from": "alfven128_lowkz_f0p02_eta100/checkpoints/checkpoint_t2000.0.h5"},
]
...
def run_hermite_branch(
    ...
    """Run 128^3 Alfvenic + Hermite cascade at the requested M."""
    ...
    # ---- Inlined low-k_z Hermite forcing (mirrors modal_128_hermite.py) ----
    ...
    # ---- Fixed physics (match modal_128_hermite.py) ----
    eta = 100.0
    hyper_r = 2
    hyper_n = 6
    cfl_safety = 0.3
    ...
    print(f"  Original M={state.M}, expanding to M={M}")

    old_g = jnp.array(state.g)
    M_old = state.M
    new_g = jnp.zeros(
        (old_g.shape[0], old_g.shape[1], old_g.shape[2], M + 1),
        dtype=old_g.dtype,
    )
    new_g = new_g.at[:, :, :, :M_old + 1].set(old_g)

    seed_g = initialize_hermite_moments(
        grid, M, perturbation_amplitude=1e-3, seed=137,
    )
    ...
    print(f"\n{'='*70}")
    print(f"128^3 Alfvenic + Hermite Cascade: {label}")
    print(f"{'='*70}")
    print(f"  eta={eta}, nu={nu}, hyper_r={hyper_r}, hyper_n={hyper_n}")
    print(f"  M={M}, Alfven f={force_amplitude}, Hermite f={hermite_amplitude}")
    print(f"  forced_moments={forced_moments}")
    print(f"  dt={dt:.6f}, total_time={total_time} tau_A")
        ...
        if step % diagnostic_interval == 0:
            ...
            if step % (diagnostic_interval * 10) == 0:
                ...
                print(
                    ...
                    f"E_tot={etotal:.4e}  eps_nu={eps_nu:.4e}  "
                    f"SumW_m={hermite_total:.4e}  wall={wall_elapsed:.0f}s"
                )
    ...
    print(f"  eps_nu var={rel_var:.1%}")
    if eps_nu_history:
        print(f"  eps_nu final={eps_nu_history[-1]:.4e}")
...
def main():
    """Submit M-convergence runs."""
    print(f"Submitting {len(BRANCHES)} M-convergence branches at nu=3...")
    for b in BRANCHES:
        print(f"  {b['label']}: M={b['M']}, nu={b['nu']}, "
              f"total_time={b['total_time']} (avg from t={b['averaging_start']})")
    ...
    print(f"\n{'='*80}")
    print("M-CONVERGENCE RESULTS")
    print(f"{'='*80}")
    for r in results:
        print(f"  {r['label']} (M={r['M']}, nu={r['nu']}): {r['verdict']}, "
              f"eps_nu={r['eps_nu_final']:.4e}, var={r['eps_nu_variation']:.1%}, "
              f"t={r['final_time']:.0f}, wall={r['wall_time']/3600:.1f}h")
