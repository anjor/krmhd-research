...
- **Paper draft complete pending review (2026-05-17).** The dissipative-anomaly paper (`paper/dissipative-anomaly/main.tex`, 7 pages) is now a full draft: §4 Discussion (self-adjustment / ν≈3 kink, finite-M caveat, collisionless-limit scope, Zocco-Schekochihin comparison), Appendix A (Lawson-RK4 instability diagnosis + IMEX-RK222 fix), Acknowledgements, and Figure 3 are all written. Figure 3 (`hermite128_nu3_imex_basestate_wmt.{pdf,png}`) shows the base-state E(k⊥) — a short k⊥^{-1.43} inertial range, close to k⊥^{-3/2} — and the W(m,t) heatmap with no truncation pileup; generated by the new committed script `analysis/plot_paper_figure3.py` from the ν=3 IMEX spectra fetched off the Modal volume. Bibliography polished with verified DOIs/arXiv IDs; document kept on the `article` class (JPP conversion deferred to submission). Outstanding before submission: confirm acknowledgement names/credits; the long ν=3 equilibration run and Π(m) flux cross-check (Next Steps 3–5 below) remain open and would strengthen the energy-balance argument.
- **Review-round revisions (2026-05-20).** Round-trip review on the draft surfaced four concrete revisions, all now in. (1) Prior-work citation gap filled: added a comprehensive paragraph in §1 citing Schekochihin+2016 (phase mixing vs nonlinear advection, JPP), Adkins-Schekochihin 2018 (solvable 1D Vlasov-Hermite model, m^{-3/2} prediction), Meyrand+2019 (first nonlinear driven 3D KRMHD sim, single-ν/M), Eyink 2018 (PRX, theoretical framework for collisionless dissipative anomalies), and Nastac+2024 (phase-space entropy cascade and dissipative anomaly in gyrokinetics) — what our paper adds is the explicit factor-of-50 ν-scan in KRMHD. (2) Figure 1 right panel replaced: was total Hermite energy vs ν (auxiliary), is now the Hermite cascade flux Π(m) for the six runs computed via GANDALF's `hermite_flux` on t=2180,2190,2200 τ_A checkpoints. New committed script `analysis/plot_paper_figure1_with_pi.py`. Result: Π(m) is approximately constant in m and approximately ν-independent across m∈[4,40], with Π_inertial = 30.8. The injection-point value Π(0) ≈ 55 ≠ ε_ν = 49 because energy returns to the Alfvénic sector through the (1-1/Λ²) coupling at the g_0↔g_1 boundary; the constant-flux value is what gives the ν-independent plateau. (3) §4 Discussion rewritten to drop the muddled m_d-vs-M ordering framing and use the standard constant-flux cascade picture instead (inertial range oblivious to ν as ν→0; injection sets dissipation). (4) Nonlinearity sentence rewritten — the simulations are nonlinear, so the question is whether we have enough dynamic range to see the Adkins-Schekochihin m^{-3/2} echo regime (we don't, at our resolution). Paper now 8 pages, compiles cleanly. Committed on `paper/finish-dissipative-anomaly`.
- **Second-round revisions (2026-05-21).** User flagged that the Π(m) panel had real numerical artifacts at ν=1 (pileup) and dropped it; reverted Figure 1 to single-panel ε_ν(ν), updated §3.1 and §4 to drop the constant-flux-measurement claim, and reframed the imbalanced-turbulence follow-up as part of the Meyrand-line work the author is in. Then, on user direction, ran the M-convergence study as part of this paper: new committed runner `scripts/modal_M_convergence.py` launched two A100 runs from the same Alfvénic checkpoint — `hermite_M64_nu3_imex` (M=64, 200 τ_A, ~3 h wall) and `hermite_M256_nu3_imex` (M=256, 100 τ_A, ~7.5 h wall, shortened to fit one 10 h Modal session at twice the per-step cost). Results: ε̄_ν = {49.11±10.99, 49.21±10.65, 50.28±9.92} at M = {64, 128, 256} — 2.4% spread, well inside the per-run scatter. ΣW_m grows monotonically with M (144 → 162 → 198) as the hyper-collisional cutoff $\nu(m/M)^6$ moves outward but the integrated dissipation is unchanged. New §3.3 "Hermite-resolution convergence" added with a 3-row table; abstract updated to mention the M check; Discussion caveat about "M=64 and M=256 follow-up" replaced with a forward reference to §3.3.
