...
- **128³ benchmark launched on Modal (2026-04-01).** Three branches submitted in parallel on A100 GPUs via `scripts/modal_128_benchmark.py`, each running for 200 τ_A with checkpoints every 10 τ_A. The parameter choices come directly from the overnight 64³ calibration: `eta=4.0, f=0.003` (best 64³ survivor), `eta=3.0, f=0.004` (middle), and `eta=2.0, f=0.005` (baseline). All use **upstream GANDALF shell forcing** (`force_alfven_modes_gandalf`), not the study-local low-k_z wrapper — this keeps the 128³ calibration consistent with the exact benchmark path that produced the 64³ parameter recommendations. Output is on Modal volume `krmhd-benchmark-vol`; download with `scripts/download_128_results.py`.
- **What to do when the 128³ runs finish.** (1) Download results: `uv run python studies/02-collisionality-scan/scripts/download_128_results.py`. (2) Check energy time series (`energy_timeseries.npz`) for each branch — look for a plateau in E_total during the second half (t > 100 τ_A). (3) Plot E(k⊥) spectra from the averaging window (`spectra/`) and check for a clean k⊥^{-5/3} inertial range. The key question is whether 128³ provides enough scale separation between forcing (n=1–2) and dissipation to sustain a proper cascade. If one branch achieves steady-state Alfvénic turbulence, that checkpoint becomes the starting point for adding Hermite forcing and running the ν-scan.
- **Decision point after 128³ results.** If at least one branch shows a clean inertial range and steady E_total: proceed to Hermite forcing (add `g_0` forcing, set M=128, hyper_n=6, pick a ν). If all branches blow up or remain too steep: the problem is likely not resolution but the forcing/dissipation balance, and we may need to revisit the forcing scheme (e.g., switch to low-k_z, try balanced Elsasser, or increase eta further).
