...
\caption{\textbf{Dissipative anomaly and constant-flux cascade.} Left: time-averaged collisional dissipation $\bar\epsnu$ as a function of $\nu$, from six IMEX runs at $M=128$, $128^3$, $200\,\taua$. Blue circles are time-series means with $\pm 1\sigma$ error bars; red diamonds are spectrum-window means. The horizontal dashed line at $\bar\epsnu = 49.09$ is the six-point mean. Gray squares are legacy Lawson-RK4 short-probe points ($50\,\taua$ before numerical blowup set in; see Appendix~A) and are shown for reference only. Right: time-averaged Hermite cascade flux $\Pi(m) = \sum_{\mathbf{k}} \Gamma_{m,\mathbf{k}}$, where $\Gamma_{m,\mathbf{k}} = -k_\parallel \sqrt{2(m+1)}\,\mathrm{Im}[g_{m+1} g_m^*]$, averaged over checkpoints at $t = 2180, 2190, 2200\,\taua$. Colours run from $\nu = 1$ (purple) to $\nu = 50$ (yellow). The inertial range $m \in [4, 40]$ is shaded; in that range $\Pi(m)$ is approximately constant in $m$ and approximately the same across all six $\nu$, with a six-point mean $\Pi_\mathrm{inertial} = 30.8$. Beyond the inertial range the curves split with the dissipation-scale ordering: lower $\nu$ carries the cascade further out in $m$ before collisions absorb it.}
\label{fig:plateau}
\end{figure}

The right panel of Figure~\ref{fig:plateau} shows the mechanism directly. In the inertial range $m \in [4, 40]$, $\Pi(m)$ is approximately constant in $m$ and approximately the same across the six $\nu$, with a six-point inertial-range mean of $\Pi_\mathrm{inertial} = 30.8$. This is the constant-flux signature of a turbulent cascade in moment space: the rate at which energy is delivered to high $m$ is set by the inertial-range dynamics, not by the dissipation mechanism. Beyond $m \approx 40$ the curves split in the expected order --- the lowest-$\nu$ run carries the cascade out to $m \gtrsim 100$, while the highest-$\nu$ run sees the flux extinguished by collisions before $m \approx 80$. The injection-point value $\Pi(0) \approx 55$ is larger than $\Pi_\mathrm{inertial}$ by about a factor of two; the difference is energy returned to the Alfv\'enic sector through the $(1 - 1/\Lambda^2)$ coupling at the $g_0 \leftrightarrow g_1$ boundary and does not appear in the integrated $\bar\epsnu$, which is set by the inertial-range flux. The total Hermite energy $\langle \sum_m W(m) \rangle_t$ in Table~\ref{tab:scan} shows the corresponding self-adjustment of the spectrum at the dissipation scales: as $\nu$ decreases, more total Hermite energy is carried (by $\sim 3\times$ from $\nu = 50$ to $\nu = 1$) while $\bar\epsnu$ stays fixed.
...
The plateau in $\bar\epsnu(\nu)$ is the standard signature of a constant-flux turbulent cascade. A driven turbulence problem separates into three scale ranges: a forcing range, an inertial range, and a dissipation range. In the inertial range, energy is transferred between scales without being removed, so the flux through any intermediate scale is constant in scale --- it is set by the inertial-range dynamics, not by the dissipation mechanism. As $\nu$ decreases, the dissipation range retreats to higher $m$, but this inertial-range flux is unchanged. The integrated collisional dissipation $\bar\epsnu$, which is whatever the inertial range delivers to the damped scales, therefore approaches a $\nu$-independent value. The Hermite cascade flux $\Pi(m)$ in the right panel of Figure~\ref{fig:plateau} is the direct moment-space measurement: in the inertial range $m \in [4, 40]$, $\Pi(m)$ is approximately constant in $m$ and approximately the same across all six $\nu$. That is the cascade content of the plateau in $\bar\epsnu$.
